...
8. **Asumsi Lokasi Folder Desktop di Windows Modern (OneDrive)**: Setelah memperbaiki isu hak akses ke `USERPROFILE\Desktop`, ternyata file masih tidak terlihat oleh pengguna. Saya gagal menyadari bahwa pada Windows 10/11 modern, folder Desktop seringkali dipindahkan otomatis oleh sistem ke dalam `C:\Users\Username\OneDrive\Desktop`. Menulis langsung ke *path* *hardcoded* `USERPROFILE\Desktop` menyebabkan file tersimpan di folder *dummy* yang tidak tampil di layar. Solusi yang jauh lebih aman dan sesuai standar UX adalah mengarahkan seluruh unduhan sistem ke folder `Downloads` (`USERPROFILE\Downloads`).
9. **Kerapuhan `sys.argv[0]` dan `sys.executable` pada Subprocess (WinError 2)**: Pada iterasi pertama perbaikan *Lock Screen*, saya mengandalkan `sys.argv[0]` untuk mencari `main.py`. Jika program dijalankan dari *working directory* yang berbeda, `sys.argv[0]` (berupa _relative path_) menjadi tidak valid dan memicu *WinError 2: File not found* saat dilempar ke `subprocess.Popen`. Selain itu, pada beberapa distribusi Windows Store, `sys.executable` bisa bernilai kosong/None. Solusi perbaikannya adalah selalu menggunakan path absolut `os.path.abspath(__file__)` untuk mencari `main.py`, memberikan fallback `python` jika `sys.executable` kosong, serta menyisipkan *debug logger* (`cmd_str`) di balasan WebSocket untuk mempermudah pelacakan jika error serupa muncul lagi.
10. **Crash Tersembunyi pada PyInstaller Subprocess (Missing Locker UI)**: Pada iterasi ketiga perbaikan _Lock Screen_, notifikasi di _dashboard_ mengatakan berhasil diluncurkan (karena _WinError 2_ sudah diperbaiki), tetapi GUI pengunci tetap tidak muncul. Ini terjadi karena saat PyInstaller melakukan build dengan `console=False` (tanpa jendela CMD), mencoba meluncurkan _subprocess_ baru tanpa *mengalihkan* standard input/output-nya (`stdout`, `stderr`, `stdin`) akan memicu _crash_ instan (OSException) di latar belakang karena _handle_ console-nya tidak ada/invalid. Perbaikannya adalah selalu menyuntikkan `subprocess.DEVNULL` ke semua saluran I/O dan menambahkan flag Windows `creationflags=0x08000000` (`CREATE_NO_WINDOW`) saat me-launch *sub-process* GUI pada PyInstaller. Selain itu, saya juga secara eksplisit mendaftarkan `agent.locker` ke dalam parameter `hiddenimports` di file `agent.spec` agar PyInstaller tidak tidak sengaja "membuang" module tersebut.
11. **Kesalahan Ekstraksi Parameter CLI di Windows (Pesan `--locker` Raksasa)**: Setelah UI berhasil muncul, teks yang tampil di layar klien justru bertuliskan `--locker` dan bukan pesan khusus (*custom message*) dari server. Ini terjadi karena cara kerja eksekutor Windows (`CreateProcess`) dan *bootloader* PyInstaller yang seringkali menggeser indeks `sys.argv`, sehingga algoritma parsing statis saya (`sys.argv[idx + 1]`) keliru membaca string `--locker` atau *script path* sebagai indeks pesan itu sendiri. Solusi perbaikannya adalah mengganti logika *hardcoded index* menjadi berbasis *filtering*: sistem akan membuang kata `--locker` dan nama *script* dari `sys.argv`, lalu menggabungkan sisa string yang ada sebagai teks pesan akhir. Ini membuat argument parser kebal terhadap pergeseran indeks.